...
GLFWwindow* initialize() {
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Maze generator", NULL, NULL);
    if (window == NULL) {
        glfwTerminate();
...
void set_wall(maze* maze, int2 pos, Direction dir, bool value) {
...
void gen_maze(maze* maze, int* step) {
    ...
    cell = &maze->grid[pos.x][pos.y].wall;

    } while(!(check_bit(*cell, rand_dir)));
    set_wall(maze, pos, rand_dir, false);
    step++;
}
